...
設定欄の上の「おまかせ」は主文を保ったまま見た目だけをランダムに振り直すので、方向性に迷ったときの叩き台になる。「リセット」は既定の見た目へ戻す(主文は残す)。

仕上がりは「SVGを保存」でベクタのまま、「PNGを保存」で1024pxのラスタ画像として書き出せる。「SVGをコピー」はSVGのコードをそのままクリップボードに取り出す。
